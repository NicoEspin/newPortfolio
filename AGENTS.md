...
- `src/lib/projects.ts` ya tiene contenido real (6 proyectos, ver `docs/visual-direction.md` §"Selected Work & Project detail — assets reales"). No volver a poner placeholders `[Reemplazar — ...]` ni inventar nombres, cifras o resultados — si falta un dato (ej. métricas de un proyecto), preguntarle a Nicolás en vez de fabricarlo.
